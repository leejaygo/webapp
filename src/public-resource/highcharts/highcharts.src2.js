...
// encapsulated variables
    var UNDEFINED,
        ...
        isOpera = win.opera,
        isMS = /(msie|trident|edge)/i.test(userAgent) && !isOpera,//bool,是否ie
        docMode8 = doc && doc.documentMode === 8,//bool,在ie8及以后的浏览器的特定属性（只有ie浏览器有），规定页面在ie8模式下展示
        isWebKit = !isMS && /AppleWebKit/.test(userAgent),//bool，是否为webkit浏览器
        isFirefox = /Firefox/.test(userAgent),//bool，是否为火狐浏览器
        isTouchDevice = /(Mobile|Android|Windows Phone)/.test(userAgent),//bool，是否为手机设备
        SVG_NS = 'http://www.w3.org/2000/svg',
        hasSVG = doc && doc.createElementNS && !!doc.createElementNS(SVG_NS, 'svg').createSVGRect,//bool,是否支持svg
        hasBidiBug = isFirefox && parseInt(userAgent.split('Firefox/')[1], 10) < 4, // issue #38//bool，是否火狐4以下
        useCanVG = doc && !hasSVG && !isMS && !!doc.createElement('canvas').getContext,//bool，是否仅支持canvas
        Renderer,
        hasTouch,
        symbolSizes = {},
        idCounter = 0,
        ...
        Date = win.Date,  // Allow using a different Date class
        makeTime,
        timezoneOffset,
        getTimezoneOffset,
        getMinutes,
